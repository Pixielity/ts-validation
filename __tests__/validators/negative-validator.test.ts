...
describe("NegativeValidator", () => {
  ...
  describe("validate", () => {
    it("should return true for negative numbers", () => {
      ...
      expect(validator.validate(Number.NEGATIVE_INFINITY)).toBe(true)
    })
  ...
  describe("getMessage", () => {
    it("should return appropriate error message for non-negative numbers", () => {
      expect(validator.getMessage(0)).toBe("0 is not a negative number")
      expect(validator.getMessage(1)).toBe("1 is not a negative number")
    })

    it("should include allowZero information in error message when relevant", () => {
      expect(validator.getMessage(1, { allowZero: true })).toBe("1 is not a negative number or zero")
    })

    it("should return appropriate error message for NaN", () => {
      expect(validator.getMessage(Number.NaN)).toBe("Expected a negative number but got NaN")
    })
  ...
  describe("name", () => {
    it("should return the correct validator name", () => {
      expect(validator.name()).toBe("negative")
    })
  })
})
