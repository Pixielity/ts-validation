 ...
 *
 * The NegativeValidator checks if a given value is a number less than 0,
 * or optionally less than or equal to 0 if allowZero is true.
 * It extends the BaseValidator class and implements the required
 * validate and getMessage methods.
 *
 * @extends {BaseValidator}
 * @example
 * const validator = new NegativeValidator()
 *
 ...
 */
export class NegativeValidator extends BaseValidator {
  /**
   * Creates a new NegativeValidator instance.
   *
   * Initializes the validator with the "negative" validator type.
   */
  constructor() {
    super("negative")
  }
   ...
   * @param {boolean} [options.allowZero=false] - Whether to consider zero as negative
   ...
   * const validator = new NegativeValidator()
   *
   * validator.getMessage(123)    // Returns "123 is not a negative number"
   * validator.getMessage(0)      // Returns "0 is not a negative number"
   * validator.getMessage(0, { allowZero: true }) // Returns "0 is not a negative number or zero"
   * validator.getMessage(NaN)    // Returns "Expected a negative number but got NaN"
  ...
  getMessage(value: any, options: { allowZero?: boolean } = {}): string {
    ...
    if (isNaN(value)) {
      return "Expected a negative number but got NaN"
    }

    if (options.allowZero) {
      return `${value} is not a negative number or zero`
    } else {
      return `${value} is not a negative number`
    }
  }
}
